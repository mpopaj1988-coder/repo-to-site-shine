          ...
          if (!suppressed) {
            ...
            if (supabase) {
              ...
              if (!unsubscribeToken || existing?.used_at) {
                ...
                await supabase
                  .from("email_unsubscribe_tokens")
                  .upsert({ email: data.email, token: unsubscribeToken }, { onConflict: "email" });
              }
            }
            ...
            const subject =
              typeof template.subject === "function"
                ? template.subject({ code: discountCode })
                : template.subject;
            ...
            if (lovableApiKey) {
              ...
              console.error(
                "No email path available: LOVABLE_API_KEY and SUPABASE_SERVICE_ROLE_KEY are both missing",
              );
            }
          }
        } catch (err) {
          ...
          if (mlApiKey) {
            ...
            if (!mlRes.ok) {
              ...
              if (subscriberId) {
                ...
                if (!trigRes.ok)
                  console.error(
                    "MailerLite automation trigger error",
                    trigRes.status,
                    await trigRes.text(),
                  );
              }
