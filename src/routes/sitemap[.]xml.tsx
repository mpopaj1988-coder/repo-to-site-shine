...
export const Route = createFileRoute("/sitemap.xml")({
  server: {
    handlers: {
      GET: () => {
        const staticPaths = ["/", "/properties", "/about", "/contact", "/explore", "/reviews", "/blog", "/map"];
        ...
        const urls = [
          ...
          ...guides.map(
            (g) =>
              `<url><loc>${SITE}/explore/${g.slug}</loc><lastmod>${today}</lastmod><changefreq>monthly</changefreq><priority>0.8</priority></url>`,
          ),
