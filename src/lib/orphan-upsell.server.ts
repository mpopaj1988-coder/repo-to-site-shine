...
interface PendingPair {
  ...
  orphanDate: string;
  outgoingDate: string;  // first open night — shown in outgoing (extend) message
  incomingDate: string;  // last open night before checkin — shown in incoming (early-checkin) message
  discountPct: number;
...
export async function processOrphanDayUpsells(
  ...
  for (const property of properties) {
    ...
    for (let i = 0; i < reservations.length - 1; i++) {
      ...
      // outgoingDate: the first open night after checkout — always shown to the outgoing guest
      // incomingDate: the last open night before checkin — always shown to the incoming guest
      const outgoingDate = checkoutDate;
      const incomingDate = addDays(checkinDate, -1);
          ...
          try {
            ...
            if (incomingPairForSameGuest) {
              ...
              sent = await sendMessage(
                ...
                combinedMessage(
                  name,
                  incomingPairForSameGuest.incomingDate,
                  pair.outgoingDate,
              ...
              sent = await sendMessage(
                pair.outgoing.id,
                outgoingMessage(name, pair.outgoingDate, rp, dp, staying, pair.discountPct),
                apiKey,
              );
              result.outgoingSent = sent;
            }
            ...
            if (outgoingPairForSameGuest) {
              ...
              sent = await sendMessage(
                ...
                combinedMessage(
                  name,
                  pair.incomingDate,
                  outgoingPairForSameGuest.outgoingDate,
                  earlyPrices.regularPrice,
                  earlyPrices.discountedPrice,
                  pair.discountPct,
                  latePrices.regularPrice,
              ...
              sent = await sendMessage(
                pair.incoming.id,
                incomingMessage(name, pair.incomingDate, rp, dp, pair.discountPct),
                apiKey,
              );
              result.incomingSent = sent;
            }
